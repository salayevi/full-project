...
import { ApiError, deleteAbout, getAbout, saveAbout } from "@/lib/api";
...
export function AboutForm() {
  ...
  const handleDelete = async () => {
    if (!form.id || !window.confirm("Delete the about section?")) {
      return;
    }
    ...
    try {
      await deleteAbout();
      setForm(initialAbout);
      setSuccess("About section deleted.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not delete about section.");
    } finally {
      setSaving(false);
    }
  };
