...
import { ApiError, deleteFooter, getFooter, saveFooter } from "@/lib/api";
...
export function FooterForm() {
  ...
  const handleDelete = async () => {
    if (!form.id || !window.confirm("Delete the footer section?")) {
      return;
    }
    ...
    try {
      await deleteFooter();
      setForm(initialFooter);
      setSuccess("Footer section deleted.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not delete footer section.");
    } finally {
      setSaving(false);
    }
  };
